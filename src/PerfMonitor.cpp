...
namespace pm_lib {
  ...
  void PerfMonitor::print(FILE* fp, const std::string hostname, const std::string operatorname)
  {
    ...
    fprintf(fp, "\n");
/*
 * original format for the next few lines was as below:
 * it has been modified to reflect the selective printing rule. See mail dated 2015/02/5.
 *
	fprintf(fp, "\t%-*s|    call  |        accumulated time[sec]           | [flop counts or message bytes ]\n",
		maxLabelLen, "Label");
 ...
 */
	...
	// COMM/CALC mix時はsum_flop_av/flop_serialの値が意味を失うので、speedの単位を（＊）と表示する。
    if ( (0 <= is_unit) && (is_unit <= 3) ) {
